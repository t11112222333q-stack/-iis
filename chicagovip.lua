...
MiscTab:AddToggle("Auto Farm CFrame Tốc Độ Cao", false, function(state) Config.AutoFarmLoop = state end)
MiscTab:AddSlider("Tốc Độ CFrame (Giảm nếu bị kick)", 5, 30, 12, function(value) Config.StepSize = value end)
MiscTab:AddToggle("Tự Kích Hoạt Nút (Auto Húp Box)", false, function(state) Config.AutoGetBox = state end)

MiscTab:AddToggle("Tăng Tốc Di Chuyển (Speed Hack)", false, function(state) 
    Config.Speed = state 
end)

MiscTab:AddToggle("Bật Safezone Toàn Map (Khiên Bảo Vệ)", false, function(state)
    Config.GlobalSafeZone = state
    local safeZonesFolder = workspace:FindFirstChild("SafeZones")
    if safeZonesFolder then
        for _, part in pairs(safeZonesFolder:GetChildren()) do
            if part:IsA("BasePart") then
                if state then
                    if not OriginalSafeZoneSizes[part] then
                        OriginalSafeZoneSizes[part] = part.Size
                    end
                    part.Size = Vector3.new(10000, 10000, 10000)
                    part.Transparency = 1
                    part.CanCollide = false
                else
                    if OriginalSafeZoneSizes[part] then
                        part.Size = OriginalSafeZoneSizes[part]
                    end
                end
            end
        end
