...
function ErrorPrompt(Message,state)
    if getrenv then
        ...
        prompt:setErrorTitle("MEMAYBEO HUB Error")
...
local Simple = Create("TextButton",{Parent = TopBar,BackgroundColor3 = Color3.new(1, 1, 1),AutoButtonColor = false,BackgroundTransparency = 1,Position = UDim2.new(0, 5, 0, 0),Size = UDim2.new(0, 100, 0, 18),Font = Enum.Font.SourceSansBold,Text = "MEMAYBEO HUB",TextColor3 = Color3.new(1, 1, 1),TextSize = 14,TextXAlignment = Enum.TextXAlignment.Left})
...
function newRemote(type, data)
    ...
    local connect = Button.MouseButton1Click:Connect(function()
        ...
        if blocked then
            log.GenScript = "-- THIS REMOTE WAS PREVENTED FROM FIRING TO THE SERVER BY MEMAYBEO HUB\n\n" .. log.GenScript
        end
...
newButton("Get Script", function() return "Click to copy calling script to clipboard" end, function()
    if selected then
...
newButton("Function Info",function() return "Click to view calling function information" end, function()
    ...
    if func then
        ...
        TextLabel.Text = "Done! Info generated by MEMAYBEO HUB Serializer."
    else
...
newButton("Exclude (i)", function() return "Click to exclude this Remote." end, function()
    if selected then blacklist[OldDebugId(selected.Remote)] = true TextLabel.Text = "Excluded!" end
end)

newButton("Exclude (n)", function() return "Click to exclude all remotes with this name." end, function()
...
newButton("Disable Info", function() return string.format("[%s] Toggle function info", configs.funcEnabled and "ENABLED" or "DISABLED") end, function()
...
newButton("Autoblock", function() return string.format("[%s] [BETA] Detects spammy remote calls", configs.autoblock and "ENABLED" or "DISABLED") end, function()
    configs.autoblock = not configs.autoblock
    TextLabel.Text = string.format("[%s] [BETA] Detects spammy remote calls", configs.autoblock and "ENABLED" or "DISABLED")
    history = {} excluding = {}
end)
